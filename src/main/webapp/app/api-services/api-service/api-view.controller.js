(function () {
    ...
    angular
        ...
        .controller('ApiViewController', ApiViewController);

    ApiViewController.$inject = ['CommonConstants', '$uibModalStack', '$compile', '$uibModalInstance', '$scope', '$uibModal', 'selectedApi', 'Api', 'Code', 'JhiLanguageService', 'apiPlans', 'Gateway', '$timeout', 'CommonUtil'];

    function ApiViewController(CommonConstants, $uibModalStack, $compile, $uibModalInstance, $scope, $uibModal, selectedApi, Api, Code, JhiLanguageService, apiPlans, Gateway, $timeout, CommonUtil) {

        var vm = this;
        var xsrf_token = CommonUtil.getCookie("XSRF-TOKEN");

        if (selectedApi.forPartner) {
            selectedApi = selectedApi.api;
            vm.forPartner = true;
            selectedApi.AddSettingsView = false;
            vm.mode = 'modify';
