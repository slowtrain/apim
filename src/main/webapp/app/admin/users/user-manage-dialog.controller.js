(function () {
    ...
    AdminUserDialogController.$inject = ['CommonConstants', 'CommonUtil', '$state', '$stateParams', '$uibModalInstance', 'selections', 'Principal', 'Org', 'User', 'Role', 'JhiLanguageService'];

    function AdminUserDialogController(CommonConstants, CommonUtil, $state, $stateParams, $uibModalInstance, selections, Principal, Org, User, Role, JhiLanguageService) {
        var vm = this;
        ...
        function onError(error) {
            if(vm.noSetAdmin) CommonUtil.onError(error.data.description);
                ...
                User.setAdmin({userId: vm.user.id, flag: originalFlagOfAdmin});
        ...
        function save() {
            ...
            if (selections.user) {
                User.updateForAdmin(vm.user, onSuccess, onError);
            } else {
                User.saveForAdmin(vm.user, function() { onSuccess('create') }, onError);
            }
        }
