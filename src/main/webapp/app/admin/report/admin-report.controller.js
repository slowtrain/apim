(function () {
    ...
    AdminReportController.$inject = ['$timeout', 'CommonUtil', '$scope', 'Metrics', 'AdminReportService'];

    function AdminReportController($timeout, CommonUtil, $scope, Metrics, AdminReportService) {
        ...
        AdminReportService.organizations().$promise.then(function (response) {
            vm.organizationList = response;
            AdminReportService.apps().$promise.then(function (response) {
                vm.appList = response;
                vm.appList.forEach(function (app) {
                    app.fullName = '[' + app.organization.name + '] ' + app.name;
                });
                AdminReportService.apis().$promise.then(function (response) {
                    vm.apiList = response;
                    ...
                    arraySorting(vm.organizationList, 'name');
        ...
        function uncheckingAll (list, kind, ref) {
            switch(kind){
                ...
                case 'api' : /*if (ref=='org')*/ vm.isApiAllChecked = false; vm.searchApiText = null; break;
            }
            //if (kind !='api' || ref =='org') {
        ...
        function updateList (category, ref) {
            if (category == 'app') {
                getAvailableAppList();
                ...
                getAvailableApiList();
                uncheckingAll(vm.availableApiList, 'api', ref);
                getSizeChecked('api');
            }
        }
        ...
        function checkingAllgetSize (name) {
            switch(name) {
                case 'org' : checkingAll(vm.organizationList, vm.isOrgAllChecked, vm.filterOrganization); getSizeChecked('org', false, 'checkingAll'); break;
                case 'app' : checkingAll(vm.availableAppList, vm.isAppAllChecked, vm.filterApp); getSizeChecked('app', false, 'checkingAll'); break;
                case 'api' : checkingAll(vm.availableApiList, vm.isApiAllChecked, vm.filterApi); getSizeChecked('api', 'checkingAll'); break;
            }
        }
        ...
        vm.checkingAllCheckBox = function (name) {
            switch(name) {
                case 'org' : checkingAllgetSize('org'); updateList('app'); updateList('api', 'org'); break;
        ...
        function getSizeChecked (name, alert, from) {
            switch(name) {
                case 'org' :
                ...
                case 'app' :
                    ...
                    if (alert && vm.condition.appIdList.length == 0) CommonUtil.onError('기관과 앱을 먼저 선택하십시오.');
                    break;
                case 'api' :
        ...
        vm.checkingChangedInternal = function (pageMoved) {
            /*if (vm.condition.fromTime > vm.condition.toTime) {
                ...
                return;
            } else if (vm.condition.toTime - vm.condition.fromTime > 90*60*60*24*1000) {
                CommonUtil.onError('요청기간은 최대 90일입니다.');
                return;
            }*/
            getSizeChecked('org');
            getSizeChecked('app');
            ...
            if (condition.orgIdList.length >0 && condition.appIdList.length >0){
                CommonUtil.underLoading();
                if (!pageMoved && vm.availableApiList.length == condition.apiIdList.length) condition.apiIdList = [];
