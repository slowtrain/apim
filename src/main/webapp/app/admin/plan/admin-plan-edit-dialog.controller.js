(function() {
    ...
    angular
        ...
        .controller('AdminPlanEditDialogController', AdminPlanEditDialogController);

    AdminPlanEditDialogController.$inject =['CommonUtil', '$scope', '$filter', '$log', '$stateParams', '$uibModalInstance', 'JhiLanguageService', 'selectPlan', 'apiPlans'];

    function AdminPlanEditDialogController(CommonUtil, $scope, $filter, $log, $stateParams, $uibModalInstance, JhiLanguageService, selectPlan, apiPlans) {
...
//                    AdminAccountPlan.update(vm.plan, function() { onSuccess('update') }, onError);
//                }else{
//                    AdminAccountPlan.create(vm.plan, function() { onSuccess('create') }, onError);
//                }
            }else{
                ...
                if(selectPlan.data != null){
                    apiPlans.update({isGlobal : true}, vm.plan, function() { onSuccess('update') }, onError);
                }else{
                    apiPlans.save({isGlobal : true}, vm.plan, function() { onSuccess('create') }, onError);
                }
            }
        }
        ...
        numeral.zeroFormat(''); //값이 0 일 때 빈칸으로 표시한다. (has-error를 적용시키기 위해)
        numeral.nullFormat(''); //값이 null일 때 빈칸으로 표시한다. (has-error를 적용시키기 위해)
