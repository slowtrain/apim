...
angular
    ...
    .controller('swaggerUiController', ["$scope", "$http", "$location", "$anchorScroll", "$timeout", "swaggerClient", "swaggerModules", "swaggerTranslator", function ($scope, $http, $location, $anchorScroll, $timeout, swaggerClient, swaggerModules, swaggerTranslator) {
        ...
        $scope.submitExplorer = function (operation) {
            ...
            swaggerClient
                .send(swagger, operation, $scope.form[operation.id], $scope.$parent.httpHeaders)
        ...
        $scope.auth = function (operation) {
            ...
            swaggerModules
                .execute(swaggerModules.AUTH, swagger, operation, auth)
                .catch(onError);
        };
        ...
        $scope.getSourceCode = function (operation, language) {
            var values = $scope.form[operation.id];
            var query = {},
                headers = {},
                path = operation.path,
                body;
            ...
            if ($scope.$parent.httpHeaders) {
                $scope.$parent.httpHeaders.forEach(function (header) {
            ...
            var options = {
                method: 'POST',
                url: '/api/swagger/code/generate',
                data: datas
                // data: {
                //     url: $scope.url,
                //     // values : $scope.form[op.id],
                //     operation: operation,
                //     lang: language
                // }
            };
            ...
            $http(options)
                .then(function successCallback(response) {
                    operation.source_code = response.data.result;

                    var codes = document.getElementsByClassName('prettyprinted');

                    for (var i = 0; i < codes.length; i++) {
                        angular.element(codes[i]).removeClass('prettyprinted');
                    }
        ...
        this.send = function (swagger, operation, values, customHeaders) {
            ...
            var basePath = swagger.basePath || '',
                ...
                callback = function (response) {
                    ...
                    // 화면 표시를 위해 원본 URL로 다시 치환한다.
                    response.config.url = originalUrl;
                    swaggerModules
            ...
            // API Portal로 요청하기 위해 기본 URL을 변경한다.
            var originalUrl = options.url;
            options.url = '/api/swagger/proxy/call-api';
