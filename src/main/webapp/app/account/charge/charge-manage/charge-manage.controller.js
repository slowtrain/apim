(function () {
    ...
    angular
        ...
        .controller('ChargeManageController', ChargeManageController);

    ChargeManageController.$inject = ['Billing', '$scope', '$rootScope', 'BillingCharge', 'AdminReportService', '$uibModalStack', '$uibModal'];

    function ChargeManageController(Billing, $scope, $rootScope, BillingCharge, AdminReportService, $uibModalStack, $uibModal) {
        ...
        vm.downloadExcel = function () {
            var condition = $.extend({providerId: vm.user.organization != null ? vm.user.organization.id : null}, vm.condition);
            if (condition.chargeMonth) condition.chargeMonth = condition.chargeMonth.getFullYear()+""+(condition.chargeMonth.getMonth()<9?"0"+(condition.chargeMonth.getMonth()+1) : condition.chargeMonth.getMonth()+1);

            var queryString = $.param(condition, true);
            window.open('api/charge/excel/providing?' + queryString, "_blank");
        };
    }
})();
