(function () {
    ...
    UserManageController.$inject = ['$scope', 'CommonUtil', '$uibModal', 'Principal', 'User', 'Role', '$stateParams', 'JhiLanguageService'];

    function UserManageController($scope, CommonUtil, $uibModal, Principal, User, Role, $stateParams, JhiLanguageService) {
        var vm = this;
        ...
        vm.searchColumns = [{name: "ID", ko: "아이디"}, {name: "NAME", ko: "닉네임"}, {name: "EMAIL", ko: "이메일"}];
        ...
        function setActive(user) {
            ...
            var callback = function(result) {
                if (result) {
                    ...
                    User.update(user, function(){ vm.loadAll() }, onError);
                }
            }
        ...
        function loadAll(page) {
            ...
            User.query(vm.condition, onSuccess, onError);
        }
        ...
        function showDetail(user) {
            $uibModal.open({
                templateUrl: 'app/account/user/user-manage/user-manage-detail.html',
                controller: 'UserManageDialogController',
                controllerAs: 'vm',
                backdrop: 'static',
                size: 'md',
                resolve: {
        ...
        function editUser(user) {
            $uibModal.open({
                templateUrl: 'app/account/user/user-manage/user-manage-dialog.html',
                controller: 'UserManageDialogController',
                controllerAs: 'vm',
                backdrop: 'static',
                size: 'md',
        ...
        function delUser(user) {
            ...
            $uibModal.open({
                ...
                if (result) User.delete({login: user.login}, function() { loadAll(0) }, onError);
            });
        }
