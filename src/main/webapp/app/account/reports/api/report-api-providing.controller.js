(function () {
    ...
    angular
        ...
        .controller('ReportApiProvidingController', ReportApiProvidingController);

    ReportApiProvidingController.$inject = ['$timeout', 'CommonUtil', '$scope', 'Metrics', 'ReportService'];

    function ReportApiProvidingController($timeout, CommonUtil, $scope, Metrics, ReportService) {
        ...
        vm.condition = {
            ...
            size: vm.size, //초기 로딩시에만 이 값이 입력된다. 나중에 vm.size를 변경한다면, 다시 vm.condition.size = vm.size; 라는 명령을 내려야 한다.
            page: vm.page
        };  // 검색조건
        ...
        ReportService.organizations({orgType: 'provider'}).$promise.then(function (response) {
